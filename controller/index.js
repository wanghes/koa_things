...
const { aesEncode, aesDecode, genSign, deSign } = encrypto;
...
const auth = async (ctx, next) => {
    if (
        ctx.request.path == "/insertUser" ||
        ctx.request.path == "/articles" ||
        ctx.request.path == "/main"
    ) {
        //便于插入数据
        await next();
        return;
...
const getToken = (name, password, id) => {
    return new Promise((resolve, reject) => {
        ...
        jwt.sign(
            data,
            config.tokenSecret,
            { expiresIn: 60 * 60 * 240 }, //登录token设置24小时过期时间
            (err, token) => {
                if (err) {
                    reject(err);
                } else {
